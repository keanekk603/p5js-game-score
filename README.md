...
- once that is done we change the id of cell b to be === cell a. this conditions ensures that the condition if (CellA.id != CellB.id) is not called. we join the cells together essentially, by making the id of cella and cell b the same. then we remove that walls from the array walls= [] throguh the splice, this doesnt mean all the wall will be removed it only means that the wall between the cell will be remove if the cell id is different as shown in the code, but in the end the walls shld be empty. this also ensures that there are no disjointed rooms a bug which i was stuck on for quite a while. it goes throguh every single cell until it at least connected to one another ensuring no disjointed rooms.
